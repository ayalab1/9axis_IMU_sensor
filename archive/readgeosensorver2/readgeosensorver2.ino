...
if (mpu.update())
{
    ...
    geodata.LinearAccX = mpu.getLinearAccX();
    geodata.LinearAccY = mpu.getLinearAccY();
    geodata.LinearAccZ = mpu.getLinearAccZ();
}
    if(Serial.available()){
    byte a = Serial.read();
    if(a == 'h'){
   Serial.write((byte *) &geodata, (17*4+1));
   ...
   digitalWrite(triggerPin, LOW);
    ...
    +String(mpu.getGyroBiasX() / (float)MPU9250::CALIB_GYRO_SENSITIVITY)+","
    +String(mpu.getGyroBiasY() / (float)MPU9250::CALIB_GYRO_SENSITIVITY)+","
    +String(mpu.getGyroBiasZ()/ (float)MPU9250::CALIB_GYRO_SENSITIVITY)+","
